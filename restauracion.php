...
<section>
    ...
    <div id="login">
        ...
        <form class="formlogin" name="clientes" autocomplete="off" action="" method="POST">			  
            <div class="flogin">
                <label><p>Ingrese su Nombre de Usuario</p></label>
                <input type="text" name="usuario"/>
            </div>
            <div class="botonlogin">
                <button class="boton_envia" type="submit" input="submit" name="activacion" value="Restaurar"><p>Restaurar</p></button>
            </div>
        </form>
        ...
        <?php	
                if(isset($_POST['activacion'])){
                    ...
                    $datos = mysqli_query($conexion, "SELECT * FROM usuarios WHERE usuario = '$usuario'");
                    ...
                    if ($usuario == $usuario_base) {
                        ...
                        $pagina_web = "http://" . $host . $url;
                        $desarmado = pathinfo($pagina_web);
                        $link = $desarmado['dirname']."/".$desarmado['filename'].".cod.php?codigo=".$codigo;
                        $de = "Soporte Laialy";
                        $asunto = "Restauración de Contraseña";
                        $titulo = "Soporte Laialy";
                        $parrafo = $nombre." ".$apellido.", hemos recibido una solicitud de restauración de contraseña.<br>Si usted solicito esta restauración haga click en el siguiente link.<br>".$link."<br>Si usted no lo solicito, ignore este mensaje.";
                        ...
                        if($mail->Send()) {
                            ...
                            require("../conexion.laialy.php");
                            mysqli_query($conexion, "UPDATE usuarios SET activacion='$codigo' WHERE usuario = '$usuario'");
                            mysqli_close($conexion);
                            echo "<script language=Javascript> location.href=\"index.php?mensaje=restauracion\"; </script>";
                        } else {
                            //Sacamos un mensaje con el error.
                            echo "<script language=Javascript> location.href=\"restauracion.php?mensaje=restauracion.error\"; </script>";
                        }                        
                    } else {
                        echo "<script language=Javascript> location.href=\"restauracion.php?mensaje=restauracion.error\"; </script>";                        
                    }                             
